...
// --- CONFIGURAÇÃO DO FIREBASE (OFICIAL ARENA HENKO) ---
...
// ID FIXO para garantir sincronização entre PC e Celular
const appId = "arena-henko-v2-prod";
  ...
  const renderHome = () => (
    <>
      <section className="relative h-[90vh] flex flex-col items-center justify-center overflow-hidden bg-black text-center">
        ...
        <div className="relative z-10 px-4 text-white w-full max-w-4xl">
          ...
          <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-2xl mx-auto relative z-20">
              ...
              <button onClick={() => window.open(getWaLink(`Tenho interesse nos shows Arena Henko`))} className="bg-neutral-900/80 backdrop-blur-md border border-neutral-800 p-4 rounded-2xl flex items-center gap-4 hover:border-red-600 transition-all group text-left w-full text-white">
      ...
      <section id="calendario" className="py-32 px-4 text-white text-center bg-neutral-950">
        <div className="max-w-7xl mx-auto">
          <div className="mb-16">
            <h2 className="text-5xl font-black uppercase tracking-tighter text-white">Temporada <span className="text-red-600">2026</span></h2>
          ...
          <div className="flex flex-wrap gap-2 mb-12 justify-center text-white">
            {SPORT_DATA.map(s => (
              <button key={s.id} onClick={() => setActiveSportId(s.id)} className={`px-8 py-4 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${activeSportId === s.id ? 'bg-red-600 text-white shadow-xl scale-105 font-bold' : 'bg-neutral-900 text-gray-500 hover:text-gray-300'}`}>{s.name}</button>
            ))}
          </div>
          {selectedSport && (
              ...
              <div className="grid lg:grid-cols-5 gap-12 items-center text-center">
                ...
                <div className="lg:col-span-3 w-full text-white font-bold">
                  ...
                  <div className="space-y-3">
                      ...
                      <div key={i} className="bg-neutral-950/50 border border-neutral-800 rounded-[24px] overflow-hidden hover:border-red-600/30 transition-all relative text-white text-left font-bold">
                        <button onClick={() => setExpandedMatchKey(expandedMatchKey === i ? null : i)} className="w-full p-5 flex items-center justify-between text-white text-left font-black">
                          <div className="flex items-center gap-4 lg:gap-6">
                            ...
                            {m.scarcity && (
                              <div className="ml-2 hidden sm:flex items-center gap-1 bg-red-600/10 border border-red-600/30 px-2 py-1 rounded-full animate-pulse text-red-500 uppercase font-black font-bold font-black"><Zap className="w-3 h-3 fill-red-500 font-black" /><span className="text-[8px] tracking-widest uppercase">{m.scarcity}</span></div>
                            )}
                          </div>
                        ...
                        <div className={`overflow-hidden transition-all duration-500 ease-in-out ${expandedMatchKey === i ? 'max-h-60 opacity-100' : 'max-h-0 opacity-0'}`}>
                          <div className="p-6 border-t border-neutral-800 bg-neutral-900/30 text-center animate-fade-in text-white font-normal">
                    ...
                    )) : <div className="p-8 border border-dashed border-neutral-800 rounded-[24px] text-center opacity-30 text-[11px] font-bold uppercase text-white font-black">Sem jogos agendados para este torneio</div>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-8 text-white text-center font-normal">
               {SHOWS_DATA.map((e, i) => (
                  <div key={i} className="flex flex-col items-center group cursor-pointer text-white font-bold" onClick={() => window.open(getWaLink(`Tenho interesse no show do ${e.name} na Arena Henko`))}>
                     ...
                     <p className="text-gray-400 text-sm font-light mb-6 uppercase tracking-widest leading-relaxed">{e.desc}</p>
                  </div>
               ))}
            </div>
         </div>
      ...
      <section id="parceiros" className="py-32 px-8 bg-neutral-950 border-y border-neutral-900 text-white text-center font-bold uppercase tracking-widest">
         <div className="max-w-7xl mx-auto text-center text-white font-bold">
            <h3 className="text-[10px] text-gray-600 font-bold uppercase tracking-[0.6em] mb-16 uppercase tracking-[0.4em]">Marcas de Elite Connosco</h3>
               ...
               {REVIEWS_DATA.map((r, i) => (
                  <div key={i} className="bg-neutral-900/50 p-10 rounded-[56px] border border-neutral-800 relative group hover:border-red-600/30 transition-all text-white">
                     ...
                     <div className="flex items-center gap-4 text-white font-bold">
                        ...
                        <div><p className="text-white text-xs font-bold uppercase tracking-wider">{r.name}</p><p className="text-gray-500 text-[10px] font-bold uppercase font-normal">{r.role}</p></div>
                     </div>
                  </div>
               ))}
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 text-white text-center font-bold uppercase tracking-widest uppercase font-normal">
          {galleryPhotos.filter(p => galleryFilter === 'Todos' || p.album === galleryFilter).length > 0 ? galleryPhotos.filter(p => galleryFilter === 'Todos' || p.album === galleryFilter).map((photo) => (
            <div key={photo.id} className="aspect-[4/5] relative group overflow-hidden rounded-[56px] border border-neutral-800 bg-neutral-900 shadow-2xl transition-all hover:border-red-600/50 hover:-translate-y-3 font-normal">
              <img src={photo.image} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110 font-bold font-bold uppercase tracking-widest uppercase font-normal" alt="Moment" />
