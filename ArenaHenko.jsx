...
const App = () => {
  ...
  const renderHome = () => (
    <div className="animate-fadeIn">
      ...
      <section className="relative h-[90vh] flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 px-4 text-white w-full max-w-4xl text-center">
          ...
          <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-2xl mx-auto relative z-20">
                ...
                <button onClick={() => window.open(getWaLink(`Quero reservar o jogo: ${nextMatch.home} x ${nextMatch.away}`))} className="relative bg-neutral-900/80 backdrop-blur-md border border-neutral-800 p-4 rounded-2xl flex items-center gap-4 hover:border-red-600 transition-all group text-left w-full overflow-visible">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="text-[9px] text-red-500 uppercase font-black tracking-widest mb-1 flex items-center gap-1 font-bold"><Ticket className="w-3 h-3"/> Próximo Jogo</p>
              ...
              <button onClick={() => window.open(getWaLink(`Tenho interesse nos shows da Arena Henko`))} className="bg-neutral-900/80 backdrop-blur-md border border-neutral-800 p-4 rounded-2xl flex items-center gap-4 hover:border-red-600 transition-all group text-left w-full text-white">
                  ...
                  <div className="flex-1 min-w-0">
                      <p className="text-[9px] text-gray-400 uppercase font-black tracking-widest mb-1 flex items-center gap-2 font-bold"><Star className="w-3 h-3"/> Próximo Show</p>
                      <p className="text-sm font-bold text-white uppercase truncate">{SHOWS_DATA[0].name}</p>
      ...
      <section id="sobre" className="py-32 px-4 border-b border-neutral-900 text-center text-white bg-neutral-950">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-20 items-center text-left">
          <div className="text-white">
            <span className="text-red-600 font-black text-[10px] uppercase tracking-[0.4em] mb-4 block font-bold">EXCLUSIVIDADE</span>
          ...
          <div className="grid gap-4">
             ...
             <div className="bg-neutral-900/50 p-8 rounded-[32px] border border-neutral-800 text-left hover:border-red-900/50 transition-all group text-white">
                ...
                <h4 className="text-xl font-bold uppercase mb-2 text-white">Privacidade Total</h4>
                <p className="text-gray-500 text-sm leading-relaxed font-light">Ambiente seguro e reservado para networking de alto valor.</p>
             </div>
          </div>
        </div>
      </section>
      ...
      <section id="calendario" className="py-32 px-4 text-white text-center bg-neutral-950">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap gap-2 mb-12 justify-center text-white">
            {SPORT_DATA.map(s => (
              <button key={s.id} onClick={() => setActiveSportId(s.id)} className={`px-8 py-4 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${activeSportId === s.id ? 'bg-red-600 text-white shadow-xl scale-105 font-bold' : 'bg-neutral-900 text-gray-500 hover:text-gray-300'}`}>{s.name}</button>
            ))}
          </div>
          {selectedSport && (
              ...
              <div className="grid lg:grid-cols-5 gap-12 items-center text-center">
                ...
                <div className="lg:col-span-3 w-full text-white">
                  ...
                  <div className="space-y-3">
                    ...
                    )) : <div className="p-8 border border-dashed border-neutral-800 rounded-[24px] text-center opacity-30 text-[11px] font-black uppercase text-white font-bold">Sem jogos agendados</div>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-8 text-white text-center">
               {SHOWS_DATA.map((e, i) => (
                  <div key={i} className="flex flex-col items-center group cursor-pointer text-white" onClick={() => window.open(getWaLink(`Tenho interesse no show do ${e.name} na Arena Henko`))}>
                     ...
                     <span className="bg-red-600 text-white text-[10px] font-black px-5 py-1.5 rounded-full mb-4 uppercase tracking-widest text-white font-bold">{e.date}</span>
      ...
      <footer className="py-24 border-t border-neutral-900 opacity-50 text-white text-center relative text-white font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
  ...
  const renderGallery = () => (
    <div className="pt-32 pb-20 px-4 min-h-screen bg-neutral-950 animate-fadeIn text-center text-white font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
      <div className="max-w-7xl mx-auto text-white font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-16 text-white text-center font-bold uppercase tracking-widest uppercase font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
            <button onClick={() => setGalleryFilter('Todos')} className={`px-6 py-3 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${galleryFilter === 'Todos' ? 'bg-red-600 text-white shadow-lg font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase' : 'bg-neutral-900 text-gray-500 hover:text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase'}`}>Todos</button>
            {availableAlbums.filter(a => a !== 'Geral').map(album => (
              <button key={album} onClick={() => setGalleryFilter(album)} className={`px-6 py-3 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${galleryFilter === album ? 'bg-red-600 text-white shadow-lg font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase' : 'bg-neutral-900 text-gray-500 hover:text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase'}`}>{album}</button>
            ))}
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 text-white text-center font-bold uppercase tracking-widest uppercase font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
          {galleryPhotos.filter(p => galleryFilter === 'Todos' || p.album === galleryFilter).length > 0 ? galleryPhotos.filter(p => galleryFilter === 'Todos' || p.album === galleryFilter).map((photo) => (
            <div key={photo.id} className="aspect-[4/5] relative group overflow-hidden rounded-[56px] border border-neutral-800 bg-neutral-900 shadow-2xl transition-all hover:border-red-600/50 hover:-translate-y-3 text-white text-center font-bold uppercase tracking-widest uppercase font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-neutral-950 opacity-80 text-white text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase"></div>
              <div className="absolute top-4 right-4 z-20 text-white text-center font-bold font-bold font-bold text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
                <button onClick={() => handleDownload(photo.image, `arena-henko-${photo.id}.jpg`)} className="bg-red-600 text-white p-3 rounded-full shadow-2xl hover:scale-110 transition-transform active:scale-95 text-white font-bold text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase"><Download className="w-5 h-5 text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" /></button>
              </div>
              <div className="absolute bottom-0 left-0 right-0 p-10 text-left text-white text-left font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
                <span className="bg-red-600 text-white text-[8px] font-bold px-3 py-1.5 rounded-full uppercase tracking-widest mb-3 inline-block border border-red-500/30 backdrop-blur-sm text-white font-bold uppercase font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">{photo.album || 'Geral'}</span>
                <p className="text-white text-lg font-bold uppercase tracking-tight text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">Arena Henko Master</p>
                <p className="text-gray-500 text-[10px] font-bold mt-1 uppercase tracking-widest text-gray-500 font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">{new Date(photo.timestamp).toLocaleDateString()}</p>
              </div>
            </div>
          )) : (
            <div className="col-span-full py-40 border-2 border-dashed border-neutral-800 rounded-[5rem] flex flex-col items-center justify-center text-gray-600 bg-neutral-900/20 text-white text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
               <Camera className="w-20 h-20 mb-6 opacity-10 text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" />
               <p className="text-2xl font-bold uppercase tracking-widest text-gray-700 text-white font-bold uppercase font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">Álbum em Branco...</p>
            </div>
          )}
        </div>
      </div>
  ...
  const renderAdmin = () => (
    <div className="pt-32 pb-20 px-4 min-h-screen flex flex-col items-center bg-neutral-950 animate-fadeIn text-white text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
       {!isAdminLoggedIn ? (
         <div className="bg-neutral-900 p-16 rounded-[60px] border border-neutral-800 max-w-md w-full text-center mt-20 shadow-3xl text-white font-bold text-center text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
            <LockKeyhole className="w-20 h-20 text-red-600 mx-auto mb-10 animate-pulse text-red-600 font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" />
            <h2 className="text-3xl font-bold uppercase mb-4 tracking-tighter text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">Acesso Master</h2>
            <form onSubmit={handleAdminLogin} className="flex flex-col gap-6 text-white text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
               <input type="password" placeholder="SENHA" className="bg-neutral-950 border border-neutral-800 rounded-2xl px-6 py-6 text-white text-center font-bold outline-none focus:border-red-600 text-2xl tracking-widest transition-all placeholder:text-gray-800 text-white text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" value={adminInputPass} onChange={(e) => setAdminInputPass(e.target.value)} />
               <button type="submit" className="bg-red-600 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl hover:bg-red-700 text-white font-bold uppercase transition-all font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">Entrar</button>
               {loginError && <p className="text-red-500 text-xs font-bold mt-2 text-red-500 animate-pulse font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">{loginError}</p>}
            </form>
         </div>
       ) : (
         <div className="w-full max-w-7xl px-4 text-white text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
            <div className="flex flex-col md:flex-row justify-between items-center mb-20 gap-8 text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
                <button onClick={() => setIsAdminLoggedIn(false)} className="bg-neutral-800 text-gray-400 px-10 py-4 rounded-full font-bold uppercase text-[10px] hover:text-white transition-all shadow-xl text-gray-400 font-bold transition-all font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">Sair</button>
                <h2 className="text-4xl font-bold uppercase tracking-tighter text-white text-center font-bold uppercase uppercase font-bold uppercase tracking-widest uppercase font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">Gestão de Álbuns</h2>
                
                <div className="flex flex-col sm:flex-row gap-4 items-center bg-neutral-900 p-6 rounded-[32px] border border-neutral-800 text-white text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
                  <div className="flex flex-col gap-2 text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
                    <label className="text-[9px] font-bold uppercase text-gray-500 tracking-widest text-left ml-2 font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">Escolher Destino:</label>
                    <select value={selectedAlbum} onChange={(e) => setSelectedAlbum(e.target.value)} className="bg-neutral-950 border border-neutral-800 text-white text-xs font-bold py-3 px-4 rounded-xl outline-none focus:border-red-600 transition-all cursor-pointer font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
                      {availableAlbums.map(a => <option key={a} value={a}>{a}</option>)}
                    </select>
                  </div>
                  <div className="relative text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
                    <input type="file" accept="image/*" className="hidden" ref={fileInputRef} onChange={handleFileUpload} />
                    <button onClick={() => fileInputRef.current.click()} disabled={isUploading} className="bg-red-600 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-2xl hover:scale-105 transition-all flex items-center gap-3 text-white font-bold uppercase transition-all mt-4 sm:mt-0 font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
                      {isUploading ? <Loader2 className="w-4 h-4 animate-spin text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase"/> : <Upload className="w-4 h-4 text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase"/>} Enviar Foto
                    </button>
                  </div>
                </div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8 text-white text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
              {galleryPhotos.map((photo) => (
                <div key={photo.id} className="relative aspect-square rounded-[48px] overflow-hidden group border border-neutral-800 bg-neutral-900 shadow-xl transition-all hover:border-red-600/50 text-white text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
                   <img src={photo.image} className="w-full h-full object-cover opacity-50 font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" alt="Admin" />
                   <div className="absolute top-2 left-2 bg-black/60 px-2 py-1 rounded-lg text-[8px] font-bold text-white uppercase tracking-tighter truncate max-w-[80%] font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">{photo.album || 'Geral'}</div>
                   <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity text-white text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
                      {deleteConfirmId === photo.id ? (
                        <div className="flex gap-2 font-bold">
                           <button onClick={() => handleDeletePhoto(photo.id)} className="bg-red-600 text-white p-4 rounded-full border-2 border-white hover:scale-110 transition-all shadow-2xl text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase"><Trash2 className="w-6 h-6 text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" /></button>
                           <button onClick={() => setDeleteConfirmId(null)} className="bg-neutral-800 text-white p-4 rounded-full border-2 border-white hover:scale-110 transition-all shadow-2xl text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase"><X className="w-6 h-6 text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" /></button>
                        </div>
                      ) : (
                        <button onClick={() => setDeleteConfirmId(photo.id)} className="bg-red-600 text-white p-6 rounded-full border-4 border-neutral-950 shadow-2xl hover:scale-110 text-white font-bold text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase"><Trash2 className="w-6 h-6 text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" /></button>
                      )}
                   </div>
                </div>
              ))}
  ...
  return (
    <div className="font-sans text-gray-100 bg-neutral-950 min-h-screen selection:bg-red-600 selection:text-white overflow-x-hidden text-center text-white text-center font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
      <style>{`
      ...
      {toast && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[2000] animate-fadeInUp text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
           <div className="bg-red-600 text-white px-8 py-4 rounded-full font-black uppercase text-xs shadow-[0_0_40px_rgba(220,38,38,0.5)] text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
              <CheckCircle className="w-4 h-4 text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" /> {toast}
           </div>
        </div>
      )}

      {/* MENU Master */}
      <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-neutral-950/95 backdrop-blur-md border-b border-neutral-800' : 'bg-transparent'} h-20 flex justify-between items-center px-8 text-white text-center text-white text-white text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase`}>
        <button onClick={() => handleNavClick('home', '#')} className="hover:scale-105 transition-transform flex items-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
          <img src="https://i.imgur.com/cSYIvq6.png" className="h-8 w-auto font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" alt="Logo" />
        </button>

        <div className="hidden md:flex space-x-10 items-center text-white text-center font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
          {NAV_LINKS.map(n => (
            <button key={n.name} onClick={() => handleNavClick(n.view, n.href)} className={`text-[10px] font-bold uppercase tracking-widest transition-colors hover:text-red-500 font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase ${currentView === n.view ? 'text-red-600 font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase' : 'text-gray-300 font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase'}`}>{n.name}</button>
          ))}
          <button onClick={() => handleNavClick('home', '#contato')} className="bg-red-600 text-white px-8 py-2 rounded-full font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-all shadow-xl shadow-red-900/40 text-white font-bold cursor-pointer transition-all font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">Reservar</button>
        </div>

        <button onClick={() => setIsMenuOpen(true)} className="md:hidden text-white p-2 font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
          <MenuIcon className="w-6 h-6 text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" />
        </button>
      </nav>

      {/* Menu Mobile */}
      {isMenuOpen && (
        <div className="fixed inset-0 z-[1000] bg-neutral-950/98 backdrop-blur-3xl flex flex-col items-center justify-center space-y-10 md:hidden animate-fadeIn text-white text-center text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
          <button onClick={() => setIsMenuOpen(false)} className="absolute top-8 right-8 text-white p-3 border border-neutral-800 rounded-full text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase"><X className="w-10 h-10 text-white text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" /></button>
          {NAV_LINKS.map(n => (
            <button key={n.name} onClick={() => handleNavClick(n.view, n.href)} className="text-4xl font-black uppercase hover:text-red-500 tracking-tighter text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">{n.name}</button>
          ))}
          <button onClick={() => handleNavClick('home', '#contato')} className="bg-red-600 text-white px-16 py-6 rounded-full font-black text-sm uppercase tracking-widest shadow-2xl text-white font-bold transition-all font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">Reservar Agora</button>
        </div>
      )}
      ...
      <footer className="py-24 border-t border-neutral-900 opacity-50 text-white text-center relative text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
        <div className="absolute bottom-6 right-6 opacity-20 hover:opacity-100 transition-opacity cursor-pointer p-4 group text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">
           <LockKeyhole className="w-6 h-6 text-gray-500 group-hover:text-red-600 text-white text-white text-white text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" />
        </div>
        <img src="https://i.imgur.com/cSYIvq6.png" className="h-10 mx-auto mb-10 grayscale opacity-30 text-white text-center text-white text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase" alt="Footer Logo" />
        <p className="text-[11px] font-bold uppercase tracking-[0.5em] text-white font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold uppercase tracking-widest uppercase">Hospitalidade Arena Henko © 2026</p>
      </footer>
    </div>
  );
};
