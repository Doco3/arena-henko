...
const App = () => {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      showToast(`Sucesso! Foto no álbum: ${selectedAlbum}`);
  ...
  const renderHome = () => (
    <div className="animate-fadeIn">
        ...
        <div className="relative z-10 px-4 text-white w-full max-w-4xl text-center">
          ...
          <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-2xl mx-auto relative z-20">
              ...
              <button onClick={() => window.open(getWaLink(`Interesse shows Arena Henko`))} className="bg-neutral-900/80 backdrop-blur-md border border-neutral-800 p-4 rounded-2xl flex items-center gap-4 hover:border-red-600 transition-all group text-left w-full text-white">
                   <div className="bg-neutral-800 p-3 rounded-xl group-hover:bg-red-600 transition-colors shrink-0 h-full min-h-[60px] flex items-center justify-center text-white">
      ...
      <section id="sobre" className="py-32 px-4 border-b border-neutral-900 text-center text-white bg-neutral-950">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-20 items-center text-left">
          <div className="text-white">
            ...
            <h2 className="text-5xl font-bold text-white uppercase mb-8 leading-tight">O palco da sua <br/>próxima história</h2>
      ...
      <section id="calendario" className="py-32 px-4 text-white text-center bg-neutral-950">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap gap-2 mb-12 justify-center text-white">
            {SPORT_DATA.map(s => (
              <button key={s.id} onClick={() => setActiveSportId(s.id)} className={`px-8 py-4 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${activeSportId === s.id ? 'bg-red-600 text-white shadow-xl scale-105' : 'bg-neutral-900 text-gray-500 hover:text-gray-300'}`}>{s.name}</button>
            ))}
          </div>
          {selectedSport && (
                      ...
                      <div key={i} className="bg-neutral-950/50 border border-neutral-800 rounded-[24px] overflow-hidden hover:border-red-600/30 transition-all relative text-white text-left font-bold">
                        <button onClick={() => setExpandedMatchKey(expandedMatchKey === i ? null : i)} className="w-full p-5 flex items-center justify-between text-white text-left">
                          <div className="flex items-center gap-4 lg:gap-6">
                            ...
                            {m.scarcity && (
                              <div className="ml-2 hidden sm:flex items-center gap-1 bg-red-600/10 border border-red-600/30 px-2 py-1 rounded-full animate-pulse text-red-500 uppercase font-bold"><Zap className="w-3 h-3 fill-red-500" /><span className="text-[8px] tracking-widest">{m.scarcity}</span></div>
                            )}
                          </div>
                        ...
                        {expandedMatchKey === i && (
                          <div className="p-6 border-t border-neutral-800 bg-neutral-900/30 text-center animate-fade-in text-white">
                            ...
                            <button onClick={() => window.open(getWaLink(`Reserva jogo: ${m.home} x ${m.away}`))} className="w-full bg-red-600 py-3 rounded-xl font-bold uppercase text-[10px] tracking-widest text-white shadow-xl hover:scale-[1.02] transition-all">Reservar Agora</button>
                          </div>
                        )}
                      </div>
                    )) : <div className="p-8 border border-dashed border-neutral-800 rounded-[24px] text-center opacity-30 text-[11px] font-bold uppercase text-white">Sem jogos agendados</div>}
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="py-32 bg-neutral-900/30 px-4 text-white text-center text-white">
         <div className="max-w-7xl mx-auto">
            <h3 className="text-3xl font-bold text-red-500 mb-16 uppercase tracking-[0.2em] text-red-500 font-bold">Shows & Entretenimento</h3>
            <div className="grid md:grid-cols-3 gap-8 text-white text-center">
               {SHOWS_DATA.map((e, i) => (
                  <div key={i} className="flex flex-col items-center group cursor-pointer text-white" onClick={() => window.open(getWaLink(`Tenho interesse no show do ${e.name} na Arena Henko`))}>
                     ...
                     <h4 className="text-3xl font-bold uppercase mb-2 tracking-tighter text-white uppercase">{e.name}</h4>
                     <p className="text-gray-400 text-sm font-light mb-6 uppercase tracking-widest leading-relaxed font-light">{e.desc}</p>
                  </div>
               ))}
            </div>
         </div>
      ...
      <section className="py-32 px-4 bg-neutral-900/10 text-center text-white text-center text-white font-bold uppercase tracking-widest">
         <div className="max-w-7xl mx-auto text-white">
            <h3 className="text-4xl font-bold uppercase mb-16 tracking-tighter text-white uppercase font-bold text-white">A Melhor Avaliação</h3>
            <div className="grid md:grid-cols-3 gap-8 text-left text-white font-bold">
               {REVIEWS_DATA.map((r, i) => (
                  <div key={i} className="bg-neutral-900/50 p-10 rounded-[56px] border border-neutral-800 relative group hover:border-red-600/30 transition-all text-white">
                     ...
                     <p className="text-gray-300 text-sm italic mb-10 leading-relaxed font-light font-light">"{r.text}"</p>
      ...
      <footer className="py-24 border-t border-neutral-900 opacity-50 text-white text-center relative font-bold uppercase tracking-widest">
        <div className="absolute bottom-6 right-6 opacity-20 hover:opacity-100 transition-opacity cursor-pointer p-4 group text-white font-bold font-bold font-bold">
           <LockKeyhole className="w-6 h-6 text-gray-500 group-hover:text-red-600" />
        </div>
        <img src="https://i.imgur.com/cSYIvq6.png" className="h-10 mx-auto mb-10 grayscale opacity-30 text-white text-center text-white text-white font-bold font-bold font-bold uppercase tracking-widest" alt="Footer Logo" />
  ...
  const renderGallery = () => (
    <div className="pt-32 pb-20 px-4 min-h-screen bg-neutral-950 animate-fadeIn text-center text-white font-bold uppercase tracking-widest">
      <div className="max-w-7xl mx-auto text-white font-bold uppercase tracking-widest">
        ...
        <h2 className="text-5xl md:text-8xl font-black uppercase tracking-tighter mb-10 text-white text-center font-bold">Galeria da <span className="text-red-600 underline decoration-red-600/20 underline-offset-8 text-red-600">Torcida</span></h2>
        
        <div className="flex flex-wrap justify-center gap-2 mb-16">
            <button onClick={() => setGalleryFilter('Todos')} className={`px-6 py-3 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${galleryFilter === 'Todos' ? 'bg-red-600 text-white shadow-lg' : 'bg-neutral-900 text-gray-500 hover:text-white'}`}>Todos</button>
            {availableAlbums.filter(a => a !== 'Geral').map(album => (
              <button key={album} onClick={() => setGalleryFilter(album)} className={`px-6 py-3 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${galleryFilter === album ? 'bg-red-600 text-white shadow-lg' : 'bg-neutral-900 text-gray-500 hover:text-white'}`}>{album}</button>
            ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {galleryPhotos.filter(p => galleryFilter === 'Todos' || p.album === galleryFilter).length > 0 ? galleryPhotos.filter(p => galleryFilter === 'Todos' || p.album === galleryFilter).map((photo) => (
            <div key={photo.id} className="aspect-[4/5] relative group overflow-hidden rounded-[56px] border border-neutral-800 bg-neutral-900 shadow-2xl transition-all hover:border-red-600/50 hover:-translate-y-3">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-10 text-left text-white font-bold uppercase tracking-widest">
                ...
                <p className="text-white text-lg font-bold uppercase tracking-tight">Arena Henko Master</p>
       ...
       {!isAdminLoggedIn ? (
         <div className="bg-neutral-900 p-16 rounded-[60px] border border-neutral-800 max-w-md w-full text-center mt-20 shadow-3xl text-white font-bold">
            ...
            <form onSubmit={handleAdminLogin} className="flex flex-col gap-6 text-white text-center">
               ...
               <button type="submit" className="bg-red-600 text-white py-5 rounded-2xl font-bold uppercase text-xs tracking-widest shadow-xl hover:bg-red-700">Entrar</button>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-20 gap-8">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-center bg-neutral-900 p-6 rounded-[32px] border border-neutral-800">
                  ...
                  <div className="relative">
                    ...
                    <button onClick={() => fileInputRef.current.click()} disabled={isUploading} className="bg-red-600 text-white px-8 py-4 rounded-2xl font-bold uppercase text-xs tracking-widest shadow-2xl hover:scale-105 transition-all flex items-center gap-3">
    ...
    <div className="font-sans text-gray-100 bg-neutral-950 min-h-screen selection:bg-red-600 selection:text-white overflow-x-hidden text-center text-white">
      ...
      {toast && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[2000] animate-fadeInUp text-center font-bold">
           <div className="bg-red-600 text-white px-8 py-4 rounded-full font-bold uppercase text-xs shadow-[0_0_40px_rgba(220,38,38,0.5)] flex items-center gap-3">
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-neutral-950/95 backdrop-blur-md border-b border-neutral-800' : 'bg-transparent'} h-20 flex justify-between items-center px-8 text-white`}>
        ...
        <div className="hidden md:flex space-x-10 items-center font-bold">
          ...
          <button onClick={() => handleNavClick('home', '#contato')} className="bg-red-600 text-white px-8 py-2 rounded-full font-bold text-[10px] uppercase tracking-widest hover:scale-105 shadow-xl shadow-red-900/40 cursor-pointer">Reservar</button>
        </div>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[1000] bg-neutral-950/98 backdrop-blur-3xl flex flex-col items-center justify-center space-y-10 md:hidden animate-fadeIn">
          ...
          {NAV_LINKS.map(n => (
            <button key={n.name} onClick={() => handleNavClick(n.view, n.href)} className="text-4xl font-bold uppercase hover:text-red-500 tracking-tighter">{n.name}</button>
          ))}
          <button onClick={() => handleNavClick('home', '#contato')} className="bg-red-600 text-white px-16 py-6 rounded-full font-bold text-sm uppercase tracking-widest shadow-2xl">Reservar Agora</button>
        </div>
      )}
