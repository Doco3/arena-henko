...
const TEAM_LOGOS = {
  ...
  PALMEIRAS: "https://upload.wikimedia.org/wikipedia/commons/thumb/1/10/Palmeiras_logo.svg/1024px-Palmeiras_logo.svg.png"
};
    ...
    <div className="font-sans text-white bg-black animate-fadeIn overflow-x-hidden scroll-smooth">
      ...
      <nav className="fixed top-0 w-full z-[100] bg-black/60 backdrop-blur-xl border-b border-white/5 py-4 px-8 flex justify-between items-center font-black">
        ...
        <div className="hidden md:flex items-center gap-10 uppercase text-[10px] tracking-widest text-white">
            {NAV_LINKS.map(link => <a key={link.name} href={link.href} className="hover:text-red-600 transition-all duration-300 font-bold">{link.name}</a>)}
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[200] bg-black/98 backdrop-blur-3xl p-10 animate-fadeIn text-center flex flex-col gap-10 justify-center">
          ...
          {NAV_LINKS.map(link => <a key={link.name} href={link.href} onClick={() => setIsMenuOpen(false)} className="text-4xl uppercase font-black hover:text-red-600 italic">{link.name}</a>)}
        </div>
      )}
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto mb-12">
            {nextMatch && (
              <div onClick={() => window.open(getWaLink(`Interesse no jogo ${nextMatch.home} x ${nextMatch.away}`))} className="bg-white/5 backdrop-blur-md border border-white/10 p-5 rounded-3xl flex items-center gap-5 cursor-pointer text-left shadow-2xl">
                ...
                <div className="flex-1 font-bold"><p className="text-red-500 text-[10px] uppercase tracking-widest mb-1"><Zap className="w-3 h-3 inline mr-1 fill-red-500" /> Próximo Jogo</p><h3 className="text-base uppercase truncate text-white">{nextMatch.home} x {nextMatch.away}</h3><div className="text-gray-500 text-[10px] font-mono">{timeLeft.d}d {timeLeft.h}h {timeLeft.m}m</div></div>
              </div>
            )}
      ...
      <section className="py-24 bg-black border-y border-white/5 px-6 font-black text-center">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-8">
                <div className="bg-neutral-900/30 p-8 rounded-3xl border border-white/5 font-bold"><MousePointerClick className="w-8 h-8 mx-auto mb-6 text-emerald-500" /><h4 className="text-xs uppercase tracking-widest mb-4 font-bold">1. Reserve</h4><p className="text-gray-500 text-xs font-normal">Fale com o consultor oficial via WhatsApp.</p></div>
                <div className="bg-neutral-900/30 p-8 rounded-3xl border border-white/5 font-bold"><Smartphone className="w-8 h-8 mx-auto mb-6 text-emerald-500" /><h4 className="text-xs uppercase tracking-widest mb-4 font-bold">2. Receba</h4><p className="text-gray-500 text-xs font-normal">Bilhete via App oficial SPFC.</p></div>
                <div className="bg-neutral-900/30 p-8 rounded-3xl border border-white/5 font-bold"><UserCheck className="w-8 h-8 mx-auto mb-6 text-emerald-500" /><h4 className="text-xs uppercase tracking-widest mb-4 font-bold">3. Aproveite</h4><p className="text-gray-500 text-xs font-normal">Recepção VIP pela nossa equipe.</p></div>
            </div>
          </div>
      </section>
      ...
      <section id="reviews" className="py-24 px-6 bg-black text-center border-y border-white/5 text-white">
          <div className="max-w-4xl mx-auto font-bold">
            ...
            <div className="relative min-h-[300px] flex items-center justify-center overflow-hidden font-bold">
               {REVIEWS_DATA.map((r, i) => (
                 <div key={i} className={`absolute w-full transition-all duration-1000 transform ${i === currentReviewIndex ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'}`}>
                    <div className="bg-neutral-900 p-10 md:p-14 rounded-[3.5rem] border border-neutral-800 shadow-2xl relative font-bold">
                       ...
                       <div className="flex items-center justify-center gap-5">
                          <div className="w-12 h-12 bg-red-600/20 rounded-full flex items-center justify-center text-red-500 font-bold">{r.initial}</div>
                          <div className="text-left font-bold"><p className="uppercase text-sm italic">{r.name}</p><p className="text-gray-600 text-[10px] uppercase tracking-widest">{r.role}</p></div>
                       </div>
                    </div>
                 </div>
               ))}
      ...
      <section id="calendario" className="py-24 px-6 bg-neutral-950 font-black text-white">
        <h2 className="text-4xl md:text-6xl font-black uppercase text-center mb-16 italic font-bold">Agenda <span className="text-red-600">2026</span></h2>
            ...
            <div className="grid lg:grid-cols-5 gap-12 items-center font-bold">
                ...
                <div className="lg:col-span-3 space-y-4">
                    ...
                    <div key={i} className={`bg-neutral-950 border transition-all rounded-[2rem] overflow-hidden ${expandedMatchKey === i ? 'border-red-600/50 shadow-2xl' : 'border-neutral-800'}`}>
                        <button onClick={() => setExpandedMatchKey(expandedMatchKey === i ? null : i)} className="w-full p-6 flex items-center justify-between uppercase text-xs font-bold font-black">
                        ...
                        {expandedMatchKey === i && (
                           <div className="px-10 pb-10 pt-4 bg-white/5 border-t border-white/5 text-white animate-smooth">
                              <div className="grid grid-cols-2 gap-4 mb-8 text-[9px] uppercase tracking-widest font-bold">
                                  <div className="flex items-center gap-2 font-bold"><Clock className="w-3 h-3 text-red-600" /> {m.time}</div>
                                  <div className="flex items-center gap-2 font-bold"><Wine className="w-3 h-3 text-red-600" /> Open Bar</div>
                                  <div className="flex items-center gap-2 font-bold"><Coffee className="w-3 h-3 text-red-600" /> Open Food</div>
                                  <div className="flex items-center gap-2 font-bold"><Award className="w-3 h-3 text-red-600" /> VIP</div>
                              </div>
                              <button onClick={() => window.open(getWaLink(`Reserva para ${m.home} x ${m.away} no dia ${m.date}`))} className="w-full bg-red-600 py-4 rounded-2xl uppercase text-[10px] font-black hover:bg-red-700 transition-all">Garantir Ingresso Seguro</button>
                           </div>
                        )}
                    </div>
            ...
            {SHOWS_DATA.map((show, i) => (
              <div key={i} className="group flex flex-col">
                ...
                <div className="px-2 text-left">
                    <span className="text-red-600 text-[10px] font-bold uppercase tracking-[0.4em] mb-2 block">{show.date}</span>
                    ...
                    <button onClick={() => window.open(getWaLink(`Interesse oficial no evento ${show.name}.`))} className="text-[10px] font-black uppercase tracking-widest flex items-center gap-3 text-white hover:text-red-600 transition-colors group/btn uppercase"><ArrowRight className="w-4 h-4" /> Ver Disponibilidade</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="galeria" className="py-24 px-6 bg-black border-y border-white/5 font-black text-center">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col items-center justify-center mb-16 gap-4 font-bold">
            <h2 className="text-4xl md:text-6xl font-black uppercase italic text-white font-black">Nossa <span className="text-red-600">Galeria</span></h2>
            <p className="text-gray-500 uppercase text-[10px] tracking-widest font-bold font-bold">
              {isAlbumOpen ? `Explorando: ${currentAlbum.name}` : 'Selecione uma pasta para visualizar'}
            </p>
            {isAlbumOpen && (
              <button 
                onClick={() => setIsAlbumOpen(false)}
                className="mt-4 flex items-center gap-2 text-red-500 hover:text-white transition-colors uppercase text-[10px] font-bold"
              >
                <ArrowLeft className="w-4 h-4 font-bold" /> Voltar para Álbuns
              </button>
            )}
          </div>
          ...
          <div className="min-h-[400px]">
            {!isAlbumOpen ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-smooth font-bold">
                {STATIC_GALLERY.map((album) => (
                  <div 
                    key={album.id}
                    onClick={() => { setActiveAlbumId(album.id); setIsAlbumOpen(true); }}
                    className="group cursor-pointer relative overflow-hidden rounded-[3rem] aspect-[4/3] border border-white/10 bg-neutral-900 shadow-2xl font-bold"
                  >
                    <div className="absolute inset-0 font-bold">
                      <img 
                        src={album.cover} 
                        className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-60 group-hover:opacity-100 font-bold" 
                        alt={album.name} 
                      />
                    </div>
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-80" />
                    <div className="absolute bottom-10 left-10 text-left font-bold">
                       <div className="bg-red-600 px-4 py-1 rounded-full text-[8px] w-fit mb-3 uppercase font-black">Pasta</div>
                       <h3 className="text-2xl italic uppercase font-black">{album.name}</h3>
                       <p className="text-gray-400 text-[10px] mt-1 uppercase font-bold">{album.photos.length} Fotos</p>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-smooth font-bold">
                {currentAlbum.photos.map((photoName, idx) => (
                  <div key={idx} className="group relative aspect-square rounded-[2rem] overflow-hidden border border-white/5 bg-neutral-900 shadow-xl font-bold">
                    <img 
                      src={`${currentAlbum.folder}/${photoName}`} 
                      className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 font-bold" 
                      alt="Evento" 
      ...
      <section id="parceiros" className="py-24 bg-neutral-900/40 border-y border-neutral-900 overflow-hidden font-black">
        ...
        <div className="relative flex overflow-hidden font-bold">
          <div className="flex animate-infinite-scroll whitespace-nowrap gap-28 items-center font-bold">
            {[...PARTNERS_DATA, ...PARTNERS_DATA].map((p, i) => (
                <div key={i} className="flex-shrink-0 flex items-center justify-center w-64 h-24 transition-all duration-500 font-bold">
                    <img 
                      src={p.logoUrl} 
      ...
      <section id="faq" className="py-24 px-6 bg-black border-t border-white/5 font-black text-white text-center">
        <div className="max-w-3xl mx-auto font-bold">
            <h2 className="text-4xl font-black uppercase mb-16 italic font-bold font-bold">Dúvidas <span className="text-red-600">Frequentes</span></h2>
            <div className="space-y-4 font-bold">
                {FAQ_DATA.map((item, i) => (
                    <div key={i} className="bg-neutral-900/50 border border-neutral-800 rounded-2xl overflow-hidden text-left transition-all font-bold">
                        <button onClick={() => setExpandedFaqKey(expandedFaqKey === i ? null : i)} className="w-full p-6 flex items-center justify-between font-black uppercase text-[10px] tracking-widest group font-bold">{item.q}<ChevronDown className={`w-4 h-4 text-gray-600 transition-transform ${expandedFaqKey === i ? 'rotate-180' : ''} font-bold`} /></button>
                        {expandedFaqKey === i && <p className="px-6 pb-6 text-gray-400 text-xs font-light leading-relaxed animate-smooth font-normal font-bold">{item.a}</p>}
                    </div>
                ))}
            </div>
        </div>
          ...
          <div className="bg-neutral-900 border border-neutral-800 p-12 rounded-[3rem] w-full max-w-sm shadow-3xl text-center font-bold">
            ...
            <form onSubmit={handleAdminLogin}>
              <input 
                ...
                className="w-full bg-black border border-neutral-800 rounded-2xl px-8 py-5 mb-6 text-white focus:outline-none focus:border-red-600 text-center tracking-widest font-bold" 
              />
              <div className="flex gap-4 font-bold">
                <button type="button" onClick={() => setIsLoginModalOpen(false)} className="flex-1 py-4 text-[10px] uppercase border border-neutral-800 rounded-2xl font-bold">Voltar</button>
                <button type="submit" className="flex-1 py-4 text-[10px] uppercase bg-red-600 rounded-2xl text-white font-bold font-bold">Entrar</button>
              </div>
            </form>
          </div>
        </div>
      )}
      {toast && <div className="fixed bottom-12 left-1/2 -translate-x-1/2 z-[500] bg-red-600 text-white px-10 py-4 rounded-full font-black text-[10px] uppercase tracking-widest animate-bounce flex items-center gap-2 font-bold font-bold"><AlertTriangle className="w-4 h-4 font-bold" /> {toast}</div>}
    </div>
  );
};
