...
const SERVICES_DATA = [
    { id: 1, title: 'Lounge de Hospitalidade', icon: <Users className="w-6 h-6" />, desc: 'Ponto exclusivo para relaxamento pré-evento.', imageUrl: 'https://i.imgur.com/Bw5Yw4K.png' },
    { id: 2, title: 'Receção VIP', icon: <Award className="w-6 h-6" />, desc: 'Atendimento premium e acesso diferenciado.', imageUrl: 'https://i.imgur.com/pLUfmBf.png' },
    ...
    { id: 5, title: 'Vista Privilegiada', icon: <MapPin className="w-6 h-6" />, desc: 'O melhor ângulo do Morumbis para os grandes shows.', imageUrl: 'https://i.imgur.com/7R1hOwg.png' },
    { id: 6, title: 'Segurança Dedicada', icon: <Shield className="w-6 h-6" />, desc: 'Equipe privada para sua total tranquilidade.', imageUrl: 'https://i.imgur.com/Gy62moQ.png' },
];
...
const BTSCountdown = () => {
  ...
  const Item = ({ value, label }) => (
    <div className="flex flex-col items-center">
      <div className="bg-black/40 backdrop-blur-md border border-white/10 w-12 h-12 md:w-16 md:h-16 rounded-xl flex items-center justify-center mb-1">
        <span className="text-lg md:text-xl font-black italic text-white">{String(value).padStart(2, '0')}</span>
      </div>
      <span className="text-[9px] uppercase tracking-widest font-black opacity-50 italic text-white">{label}</span>
    </div>
  );
  ...
  return (
    <div className="font-sans text-white bg-black animate-fadeIn overflow-x-hidden scroll-smooth font-black italic">
      ...
      <section className="relative h-screen flex flex-col justify-between bg-black overflow-hidden border-b border-white/5">
        ...
        {/* TOP: Título Reduzido para não tapar faces */}
        <div className="relative z-20 w-full max-w-7xl mx-auto px-10 pt-32 pb-10 text-center lg:text-left animate-smooth">
            ...
            <div className="flex items-center justify-center lg:justify-start gap-6 mt-2">
                <p className="text-xl md:text-2xl font-black uppercase tracking-[0.3em] text-red-600 italic">MORUMBIS</p>
                <div className="h-[1px] w-12 bg-white/10" />
                <p className="text-lg md:text-xl font-black uppercase tracking-widest text-white/40 italic">2026</p>
            </div>
        </div>

        {/* BOTTOM: Ações e Info do Camarote */}
        <div className="relative z-20 w-full max-w-7xl mx-auto px-10 py-12 flex flex-col lg:flex-row items-center justify-between gap-10">
            <div className="flex flex-col gap-2 font-black text-white/30 text-[10px] md:text-xs uppercase tracking-[0.3em] italic text-center lg:text-left">
      ...
      <section className="bg-neutral-950 py-12 px-6 border-b border-white/5">
        <div className="max-w-4xl mx-auto flex flex-col items-center font-black">
            <button onClick={() => setExpandedBtsHistory(!expandedBtsHistory)} className="bg-neutral-900 border border-white/10 px-10 py-4 rounded-full text-[11px] text-white uppercase tracking-[0.3em] font-black flex items-center gap-3 hover:text-red-600 transition-all shadow-xl">
                ...
                <div className="text-white/80 text-sm md:text-base font-normal leading-relaxed italic text-left p-8 md:p-12 bg-neutral-900 rounded-[3rem] border border-white/5 shadow-2xl mt-6 font-black">
                    <div className="grid md:grid-cols-2 gap-12 font-black">
                        <div className="space-y-6">
                            ...
                            <p className="font-normal">O BTS não é apenas o maior grupo de K-Pop, mas a maior força cultural do planeta. Composto por RM, Jin, Suga, J-Hope, Jimin, V e Jungkook, conquistaram o topo da Billboard dezenas de vezes.</p>
                            ...
                            <p className="font-normal">Em 2019, na tour 'Speak Yourself', pararam o país com dois Allianz Parque lotados em minutos. O retorno em 2026 no Morumbis será o evento da década.</p>
                        </div>
                        <div className="space-y-6">
      ...
      <section className="bg-neutral-900/50 border-y border-white/5 py-12 font-black text-emerald-500 text-[10px] tracking-widest uppercase italic font-black text-center">
        <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 md:grid-cols-3 gap-12 font-black">
            <div className="flex flex-col items-center gap-3"><ShieldCheck size={28}/><span className="font-black">CANAL OFICIAL MORUMBIS</span><StarRating /></div>
            <div className="flex flex-col items-center gap-3"><CheckCircle size={28}/><span className="font-black">ACESSO GARANTIDO</span><StarRating /></div>
            <div className="flex flex-col items-center gap-3"><Headphones size={28}/><span className="font-black">SUPORTE VIP ARENA</span><StarRating /></div>
        </div>
      </section>
      ...
      <section id="sobre" className="py-24 px-6 bg-neutral-950 border-y border-neutral-900 text-center sm:text-left font-black italic">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <div className="font-black">
            ...
            <p className="text-gray-400 text-lg mb-12 font-normal leading-relaxed italic font-black">Localizada no Morumbis, o Camarote Arena Henko oferece hospitalidade máxima e segurança total. Somos uma operação própria e oficial.</p>
          </div>
          <div className="grid gap-4">
             <div className="bg-neutral-900/40 p-10 rounded-[3rem] border border-neutral-800 flex gap-6 items-start shadow-xl group hover:border-red-600/50 transition-all font-black">
                ...
                <div className="font-black"><h4 className="text-lg font-black uppercase italic text-white font-black">Operação Oficial</h4><p className="text-gray-500 text-sm mt-2 font-normal italic font-black">Tratativa direta com o camarote. Sem intermediários.</p></div>
             </div>
          </div>
        </div>
      </section>
      ...
      <section id="servicos" className="py-24 px-6 bg-black font-black italic text-center">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl md:text-6xl font-black uppercase mb-20 italic text-white font-black">Nossa <span className="text-red-600">Experiência</span></h2>
            ...
            {SERVICES_DATA.map((service) => (
              <div key={service.id} className="group relative h-[400px] rounded-[3rem] overflow-hidden border border-white/5 bg-neutral-900 shadow-2xl transition-all hover:scale-[1.02] font-black">
                ...
                <div className="absolute bottom-10 left-10 right-10 text-left font-black">
                  ...
                  <h3 className="text-2xl font-black uppercase italic mb-2 font-black">{service.title}</h3>
                  <p className="text-gray-400 text-sm font-normal font-black">{service.desc}</p>
                </div>
              </div>
            ))}
          </div>
                    ...
                    <div key={m.id} className={`bg-neutral-950 border transition-all duration-300 rounded-[2.5rem] overflow-hidden ${expandedMatchKey === m.id ? 'border-red-600 shadow-2xl bg-neutral-900' : 'border-neutral-800 hover:border-neutral-700 font-black'}`}>
                        ...
                        <div className={`drawer-content ${expandedMatchKey === m.id ? 'drawer-open px-8 pb-8' : ''}`}>
                             <div className="pt-4 border-t border-white/5 grid grid-cols-2 gap-4 text-left font-black">
                                <div><p className="text-[10px] text-gray-500 uppercase tracking-widest font-black font-black">Horário / Local</p><p className="text-sm font-black text-white">{m.time} • Estádio do Morumbis</p></div>
        ...
        <div className="max-w-7xl mx-auto font-black">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-smooth font-black">
            {STATIC_GALLERY.map((album) => (
              <div key={album.id} className="group relative overflow-hidden rounded-[3rem] aspect-[4/3] border border-white/10 bg-neutral-900 shadow-2xl font-black">
                ...
                <div className="absolute bottom-10 left-10 text-left font-black font-black">
                   ...
                   <p className="text-gray-400 text-[10px] mt-1 uppercase font-black opacity-60 font-black">{album.count} Fotos</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <footer id="contato" className="py-32 bg-neutral-950 border-t border-neutral-900 text-center font-black italic font-black">
        ...
        <div className="grid sm:grid-cols-3 gap-6 max-w-5xl mx-auto px-6 text-white uppercase text-[10px] tracking-widest mb-20 font-black font-black font-black font-black">
            <a href="https://instagram.com/arenahenko" target="_blank" rel="noopener noreferrer" className="bg-black p-14 rounded-[3rem] border border-neutral-800 hover:border-red-600 transition-all flex flex-col items-center gap-5 shadow-2xl font-black font-black"><Instagram className="text-red-600 w-12 h-12 font-black" /> Instagram</a>
            <a href="https://wa.me/5511940741355" target="_blank" rel="noopener noreferrer" className="bg-black p-14 rounded-[3rem] border border-neutral-800 hover:border-red-600 transition-all flex flex-col items-center gap-5 shadow-2xl font-black font-black font-black"><Phone className="text-red-600 w-12 h-12 font-black" /> WhatsApp</a>
            <a href="mailto:sergio@henkoproducoes.com.br" className="bg-black p-14 rounded-[3rem] border border-neutral-800 hover:border-red-600 transition-all flex flex-col items-center gap-5 shadow-2xl font-black font-black font-black"><Mail className="text-red-600 w-12 h-12 font-black" /> E-mail</a>
        </div>
